...
function updateEditModal(productData, buttonData) {
  ...
  document.getElementById("editmodal_description").value =
  ...
  saveButton.onclick = function () {
    saveEditedProduct(productData._id, buttonData);
  };
}
...
registerButton.onclick = function (e) {
  ...
  fetch("/api/users/register", {
    ...
    if (result.status === 201) {
      ...
      alert(
        "Validation failed. Please check your inputs. username and email are uniques"
      );
...
function handleGitHubLogin() {
  const targetHref = "/api/users/github";
  window.location.href = targetHref;
}

function handleProfile() {
  const targetHref = "/api/users/profile";
  window.location.href = targetHref;
}
  ...
  if (email.includes("@")) {
    fetch(`/email/forgot-password/`, {
      ...
      .then((response) => {
        if (response.ok) {
          alert(
            `Se ha enviado un correo a '${email}', para proceder a restablecer la contraseña.`
          );
        } else {
      ...
      .catch((error) => {
        alert(
          `Ha ocurrido un error al intentar restablecer la contraseña o mail incorrecto.`
        );
      });
  } else {
...
function login() {
  const targetHref = "/api/users/login";
  window.location.href = targetHref;
}
