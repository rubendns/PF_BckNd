  ...
  init() {
    ...
    this.post(
      ...
      async (req, res) => {
        ...
        res
          .status(201)
          .send({ status: "success", message: "User crated successfully" });
      }
    );
      ...
      async (req, res) => {
        ...
        if (!user)
          return res
            ...
            .send({
              status: "error",
              error: "Wrong user/password credentials",
            });
        try {
          req.session.user = { ...user.toObject() };
          const access_token = generateJWToken(user);
          ...
          res
            .status(500)
            .send({ error: "Something went wrong, try again shortly!" });
        }
      }
    );
      ...
      try {
        ...
        req.session.destroy((error) => {
          if (error) {
            return res
              .status(500)
              .json({ error: "Error logout", msg: "Error logging out" });
          }
