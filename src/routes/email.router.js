...
import { sendForgotEmail, sendEmail } from "../controllers/email.controller.js";
...
router.post("/forgot-password/", async (req, res) => {
  try {
    const result = await sendForgotEmail(req.body.email, req.body.attachments);
    res
      .status(200)
      .json({
        message: "Correo electrónico de restablecimiento enviado correctamente",
        messageId: result.messageId,
      });
  } catch (error) {
    res
      ...
      .json({
        error:
          "Ocurrió un error al enviar el correo electrónico de restablecimiento",
      });
  }
});
...
router.post("/reset-password", async (req, res) => {
  ...
  const result = userService.updateUser(req.query.uid, {
